...
import { Track, RoomEvent, type RemoteParticipant, type LocalParticipant } from 'livekit-client';
import { VideoTrack, AudioTrack } from '@livekit/components-react';
import type { TrackReference } from '@livekit/components-core';

interface ConferenceUserViewProps {
...
export default function ConferenceUserView({
  conferenceTitle,
  onLeave,
}: ConferenceUserViewProps) {
  ...
  // Only show host tiles when they have camera/screen on or are speaking
  const visibleAdmins = useMemo(() => {
    return admins.filter((p) => {
      const camPub = p.getTrackPublication(Track.Source.Camera);
      const screenPub = p.getTrackPublication(Track.Source.ScreenShare);
      const hasCam = !!camPub && !camPub.isMuted && !!camPub.track;
      const hasScreen = !!screenPub && !screenPub.isMuted && !!screenPub.track;
      return hasCam || hasScreen;
    });
  }, [admins]);

  const allParticipants = useMemo(
    () => [...visibleAdmins, localParticipant, ...(peersVisible ? users : [])] as (LocalParticipant | RemoteParticipant)[],
    [localParticipant, visibleAdmins, users, peersVisible]
  );
  ...
  useEffect(() => {
    const onData = (payload: Uint8Array, participant?: RemoteParticipant) => {
      try {
        ...
        } else if (msg.type === 'peers_visible') {
          setPeersVisible(!!msg.enabled);
        }
      } catch {}
    };
  ...
  const toggleHandRaise = useCallback(() => {
    ...
    setRaisedHands(prev => {
      ...
      if (next) s.add(identity); else s.delete(identity);
      return s;
    });
    const msg = JSON.stringify({ type: 'hand_raise', identity, raised: next });
    ...
    <div className="flex h-screen flex-col bg-zinc-950">
      ...
      <div className="flex h-11 sm:h-12 items-center justify-between border-b border-zinc-800/60 px-3 sm:px-4 shrink-0">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
          <div className="flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-lg bg-blue-600/20 shrink-0">
            <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-blue-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="flex-1 min-h-0 relative overflow-hidden">
        ...
        {/* Hidden audio for admins not shown in grid */}
        {admins.filter(a => !visibleAdmins.includes(a)).map((p) => {
          const micPub = p.getTrackPublication(Track.Source.Microphone);
          return micPub?.track ? (
            <AudioTrack key={`audio-${p.identity}`} trackRef={{ participant: p, publication: micPub, source: Track.Source.Microphone } as TrackReference} />
          ) : null;
        })}

        {/* Hidden audio for other users (when not shown in grid) */}
        {!peersVisible && users.map((p) => {
          const micPub = p.getTrackPublication(Track.Source.Microphone);
          return micPub?.track ? (
            <AudioTrack key={`audio-user-${p.identity}`} trackRef={{ participant: p, publication: micPub, source: Track.Source.Microphone } as TrackReference} />
          ) : null;
        })}
          ...
          <div className="absolute inset-0 z-40 flex flex-col bg-zinc-950/95 backdrop-blur-sm p-1 sm:p-2">
            <div className="flex-1 min-h-0 relative overflow-hidden rounded-xl sm:rounded-2xl bg-zinc-900 ring-2 ring-white/10">
              <ParticipantTile
                ...
                size="large"
                label={spotlightParticipant.identity === localParticipant.identity ? 'You' : undefined}
              />
            </div>
            <button
              ...
              {allParticipants.map((p) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => setSpotlightIdentity(p.identity)}
                  >
                    <ParticipantTile participant={p} isLocal={isLocal} isAdmin={meta.conference_role === 'admin'} meta={meta} size="medium" label={isLocal ? 'You' : undefined} isHandRaised={raisedHands.has(p.identity)} />
                  </div>
                );
              })}
            </div>
      ...
      <div className="flex h-14 sm:h-16 items-center justify-center gap-1.5 sm:gap-2.5 border-t border-zinc-800/60 bg-zinc-900/50 px-2 sm:px-4 shrink-0">
        ...
        <ControlBtn on={isHandRaised} onClick={toggleHandRaise} tip={isHandRaised ? 'Lower hand' : 'Raise hand'}>
          <HandRaiseIcon raised={isHandRaised} />
        </ControlBtn>
        <div className="h-7 w-px bg-zinc-700/50 mx-0.5" />
        <button onClick={onLeave} title="Leave conference" className="flex h-10 sm:h-11 items-center gap-1.5 rounded-full bg-red-600 px-3 sm:px-4 text-[12px] sm:text-[13px] font-medium text-white hover:bg-red-500 active:scale-95 transition-all">
...
function ControlBtn({ children, onClick, tip, on = false, danger = false }: {
  children: React.ReactNode; onClick: () => void; tip: string; on?: boolean; danger?: boolean;
}) {
  ...
  return <button onClick={onClick} title={tip} className={`flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-full transition-all active:scale-90 ${bg}`}>{children}</button>;
}
