...
// Returns live + today's scheduled sessions for the authenticated BC
// ═══════════════════════════════════════════════════════════════
...
export async function GET(req: NextRequest) {
  ...
  // For scheduled sessions with existing rooms, get waiting participant counts from LiveKit
  ...
  await Promise.all(scheduledRows.filter(r => r.room_exists).map(async (r) => {
    ...
    try {
      ...
      // Count students only (exclude teacher, ghost roles)
