...
// ══════════════════════════════════════════════════════════════
export default function AOLiveMonitorClient({
  userName,
  userEmail,
}: {
  userName: string;
  ...
  const fetchSessions = useCallback(async () => {
    try {
      const res = await fetch('/api/v1/academic-operator/live-sessions');
    ...
    <div className="flex h-screen flex-col bg-[#1a1a1d] text-white select-none">
      ...
      <div className="flex h-13 items-center justify-between border-b border-[#3c4043] bg-[#202124] px-4 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => router.push('/academic-operator')}
                          ...
                          <div className="flex-1 overflow-hidden">
                            ...
                            <div className={sidebarTab === 'chat' ? 'h-full' : 'hidden'}>
                              <ChatPanel
                                ...
                                participantRole="academic_operator"
...
function TalkToTeacherButton() {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      await localParticipant.publishData(
        new TextEncoder().encode(JSON.stringify({ type: 'whisper_start', name: localParticipant.name || 'Academic Operator' })),
