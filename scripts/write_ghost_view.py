#!/usr/bin/env python3
"""Script to write new GhostView.tsx content"""
import os

TARGET = os.path.join(os.path.dirname(__file__), '..', 'components', 'classroom', 'GhostView.tsx')
...
export default function GhostView({
  ...
  const elapsedSec = useMemo(() => {
    ...
    if (isNaN(scheduledMs)) return 0;
    const liveMs = liveStartedAt ? new Date(liveStartedAt).getTime() : NaN;
    const effectiveStart = (!isNaN(liveMs) && liveMs > scheduledMs) ? liveMs : scheduledMs;
  ...
  const onAttentionUpdate = useCallback((msg: any) => {
    try {
      ...
      if (alertNow - lastAlert > 15_000) {
        ...
        let msg2 = ''; let severity: 'warning' | 'danger' = 'warning';
        ...
        if (msg2) { lastAlertedRef.current.set(data.studentEmail, alertNow); setAiToasts(prev => [...prev.slice(-4), { id: `${data.studentEmail}-${alertNow}`, message: msg2, severity, time: alertNow }]); }
      }
    } catch { /* ignore */ }
  }, []);
    ...
    try {
      ...
      setRaisedHands((prev) => {
        ...
        if (data.action === 'raise') {
          ...
          setAiToasts(p => [...p.slice(-4), { id: `hand-${data.student_id}-${Date.now()}`, message: `${data.student_name} raised their hand`, severity: 'info', time: Date.now() }]);
  ...
  const onMediaRequest = useCallback((msg: any) => {
    try {
      ...
      setAiToasts(prev => [...prev.slice(-4), { id: `media-${Date.now()}`, message: `${data.student_name} requested ${data.type} ${data.desired ? 'on' : 'off'}`, severity: 'info', time: Date.now() }]);
    } catch { /* ignore */ }
  }, []);
  ...
  const onLeaveRequest = useCallback((msg: any) => {
    try { const data = JSON.parse(new TextDecoder().decode(msg?.payload)); setAiToasts(prev => [...prev.slice(-4), { id: `leave-${Date.now()}`, message: `${data.student_name || 'Student'} requested to leave`, severity: 'info', time: Date.now() }]); } catch { /* ignore */ }
  }, []);
  useDataChannel('leave_request', onLeaveRequest);

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const onFullviewRequest = useCallback((msg: any) => {
    try { const data = JSON.parse(new TextDecoder().decode(msg?.payload)); setAiToasts(prev => [...prev.slice(-4), { id: `fv-${Date.now()}`, message: `${data.student_name || 'Student'} requested full view`, severity: 'info', time: Date.now() }]); } catch { /* ignore */ }
  }, []);
  useDataChannel('fullview_request', onFullviewRequest);

  useEffect(() => {
    if (aiToasts.length === 0) return;
    const t = setTimeout(() => setAiToasts(prev => prev.filter(t2 => Date.now() - t2.time < 6000)), 6000);
  ...
  return (
    <div className="flex h-screen flex-col bg-[#1a1a1d] text-white select-none">
      ...
      {/* Toast Notifications */}
      {aiToasts.length > 0 && (
        <div className="fixed top-16 right-3 z-[100] flex flex-col gap-2 pointer-events-none" style={{ maxWidth: 360 }}>
          {aiToasts.map((toast) => (
            <div
              key={toast.id}
              className={cn(
                ...
                toast.severity === 'danger' ? 'bg-red-600/90 text-white border border-red-500/50'
                  : toast.severity === 'info' ? 'bg-blue-600/90 text-white border border-blue-500/50'
